...
- [ ] 类名是否正确
- [ ] `__init__` 方法是否包含所有配置参数
- [ ] 状态变量是否初始化（position/entry_price/is_running）
- [ ] `log()` 方法是否实现
- [ ] `get_klines()` 方法是否实现
- [ ] 指标计算方法是否实现
- [ ] `check_signal()` 方法是否实现
- [ ] `open_position()` 方法是否实现
- [ ] `close_position()` 方法是否实现
- [ ] `run()` 方法是否支持 interval 参数
- [ ] `run()` 方法是否有 `self.is_running = True`
- [ ] while 循环是否有 try-except
- [ ] 是否有 KeyboardInterrupt 处理
- [ ] 启动脚本是否正确
- [ ] 是否调用 `strategy.run(interval=60)`

---

## ⚠️ 常见错误

### 错误 1: 缺少 is_running 属性
...
### 错误 4: 没有设置 is_running = True

```python
# ❌ 错误
def run(self, interval: int = 60):
    while self.is_running:  # self.is_running 初始是 False!
        pass

# ✅ 正确
def run(self, interval: int = 60):
    self.is_running = True  # 先设置为 True
    while self.is_running:
        pass
```
...
- `strategies/rsi_1min_strategy.py` - 基础 RSI 策略
- `strategies/rsi_scale_in_strategy.py` - 分批建仓策略
- `strategies/link_rsi_detailed_strategy.py` - 详细 RSI 策略

---

**文档生成时间**: 2026-03-14 21:11  
**维护负责人**: AI Assistant  
**更新频率**: 策略框架变更时更新
